        ...
        <div className="mx-auto grid max-w-[1180px] gap-6 px-4 py-5 sm:px-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end lg:px-8">
          <div className="flex items-start gap-4 sm:gap-5">
            ...
            <div className="min-w-0">
              <p className="eyebrow text-[var(--muted)]">Private Image Atelier</p>
              <h1 className="mt-2 font-display text-[2rem] leading-none text-[var(--ink)] sm:text-[2.5rem]">
                Lightframe Archive
              </h1>
              <p className="mt-3 max-w-2xl text-sm leading-7 text-[var(--ink-soft)]">
                把上传、筛选与归档收束在同一张编辑台上，减少切换成本，保留图像本身的存在感。
              </p>
            </div>
          </div>
          ...
          {authenticated ? (
            ...
            <div className="status-pill text-[var(--ink-soft)]">Visitors: locked</div>
          )}
        </div>
      </header>
      ...
      <footer className="border-t border-[var(--line)] bg-[linear-gradient(180deg,rgba(250,246,240,0.46),rgba(245,238,230,0.62))]">
        <div className="mx-auto grid max-w-[1180px] gap-4 px-4 py-6 text-sm text-[var(--muted)] sm:px-6 lg:grid-cols-[auto_1fr_auto] lg:items-center lg:px-8">
          ...
          <p className="max-w-2xl leading-7">
            一套强调层次、可读性与低噪音交互的私有图床界面，运行在 Vercel 与 Cloudflare R2。
          </p>
